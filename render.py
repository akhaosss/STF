    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign):
            ...
            if lane_marking_type == carla.LaneMarkingType.Broken or (
                ...
                marking_2 = [world_to_pixel(
                    lateral_shift(w.transform, sign * (w.lane_width * 0.5 + margin * 2))) for w in waypoints]
        ...
        def draw_lane_marking_single_side(surface, waypoints, sign):
            ...
            for sample in waypoints:
                ...
                if current_lane_marking != marking_type:
                    markings = get_lane_markings(previous_marking_type,
            ...
            last_markings = get_lane_markings(previous_marking_type,
                                             lane_marking_color_to_tango(previous_marking_color),
                                             temp_waypoints, sign)
...
class BirdeyeRender(object):
    ...
    def render_waypoints(self, surface, waypoints, world_to_pixel):
        ...
        corners = [world_to_pixel(p) for p in corners]
        pygame.draw.lines(surface, color, False, corners, 18)  # 把 10 改成 18
    ...
    def render(self, render_types=None):
        ...
        if self.hero_actor is not None:
            ...
            translation_offset = (
                hero_loc_screen[0] - self.hero_surface.get_width() / 2 + hero_front.x * self.params[
                    'pixels_ahead_vehicle'],
                hero_loc_screen[1] - self.hero_surface.get_height() / 2 + hero_front.y * self.params[
                    'pixels_ahead_vehicle']
            )
            clip_rect = pygame.Rect(translation_offset[0], translation_offset[1], self.hero_surface.get_width(),
...
class Visualizer:
    def __init__(self, world, ego_vehicle):
        ...
        pygame.display.set_caption("BEV + Camera Visualizer")
        ...
        self.birdeye_render = BirdeyeRender(world, self.bev_params, self.logger)
        self.birdeye_render.set_hero(ego_vehicle, ego_vehicle.id)

        # 相机
        self.camera = self._spawn_rgb_camera()
        self.front_cam_data = None
    ...
    def _camera_callback(self, image):
        ...
        self.front_cam_data = arr[:, :, :3][:, :, ::-1]

    def render(self, waypoints):
        self.screen.fill((0, 0, 0))
        vehicle_polys = [get_actor_polygons(self.world, 'vehicle.*')]
        walker_polys = [get_actor_polygons(self.world, 'walker.pedestrian.*')]

        self.birdeye_render.vehicle_polygons = vehicle_polys
        self.birdeye_render.walker_polygons = walker_polys
        self.birdeye_render.waypoints = waypoints

        bev_surface = self.birdeye_render.render(['roadmap', 'actors', 'waypoints'])
        target_w, target_h = SCREEN_WIDTH // 2, SCREEN_HEIGHT
        cx, cy = bev_surface.get_width() // 2, bev_surface.get_height() // 2
        rect = (cx - target_w // 2, cy - target_h // 2, target_w, target_h)

        bev_cropped = pygame.Surface((target_w, target_h))
        bev_cropped.blit(bev_surface, (0, 0), rect)
        self.screen.blit(bev_cropped, (0, 0))

        if self.front_cam_data is not None:
            cam_surface = pygame.surfarray.make_surface(self.front_cam_data.swapaxes(0, 1))
            self.screen.blit(cam_surface, (SCREEN_WIDTH // 2, 0))
